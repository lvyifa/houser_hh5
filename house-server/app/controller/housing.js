...
class Housing extends Controller {
  ...
  async housing() {
    ...
    const data = await this.app.mysql.query(
      `SELECT * FROM renthouses WHERE quyu LIKE '%${area}%' AND name LIKE '%${name}%' order by id desc limit ${
  ...
  async addHouses() {
    ...
    const sql =
      "insert into renthouses(created_at,cx,ditie,dt,fangxing,fs,img,jiage,jjr,ll,lx,lz,mianji,name,pp,qs,quyu,ts,updated_at,xq,yt,zf,zq,zx,status)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
  ...
  async setMarketBroker() {
    const { ctx } = this;
    let { pageCount = 1, pageSize = 10 } = ctx.query;
    const broker = await this.app.mysql.query(
      `SELECT * FROM brokers WHERE state LIKE '%1%'`
    );
    const data = await this.app.mysql.query(
      `SELECT * FROM brokers WHERE state LIKE '%1%' order by new_house asc limit ${
        (pageCount - 1) * pageSize
      },${pageSize}`
    );
    ctx.body = {
  ...
  async alterMarketBroker() {
    ...
    let res = await this.app.mysql.update("renthouses", {
      id,
      jjr: broker,
    });
    if (res.affectedRows) {
  ...
  async setRentHouseStatus() {
    const { ctx } = this;
    const { id, status } = ctx.request.body;
    let res = await this.app.mysql.update("secondhouses", {
      id,
      status,
    });
    if (res.affectedRows) {
  ...
  async addSecondHouses() {
    ...
    // console.log(ctx.request.body);
    const sql =
      "insert into secondhouses(created_at,cx,ditie,dt,fangxing,fs,img,jiage,jjr,ll,lx,lz,mianji,name,pp,qs,quyu,ts,updated_at,xq,yt,zf,zq,zx)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    const data = await this.app.mysql.query(sql, [
      created_at,
      cx,
      ditie,
