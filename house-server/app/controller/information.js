...
class Information extends Controller {
  ...
  async addInformation() {
    const { ctx } = this;
    const { name, tit, price, date, created_at, updated_at, status } =
      ctx.request.body;
    // console.log(ctx.request.body);
    let sql = `insert into infos (name,tit,price,date,created_at,updated_at,status) values(?,?,?,?,?,?,?)`;
  ...
  async editInformation() {
    const { ctx } = this;
    const { name, tit, date, status, id } = ctx.request.body;
    // console.log(ctx.request.body);
    let res = await this.app.mysql.update("infos", {
      name,
      tit,
      date,
      status,
